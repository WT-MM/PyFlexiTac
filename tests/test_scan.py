...
import flexitac.scripts.scan as scan_cli
from flexitac.scripts.flash import DetectedPort


def test_scan_recommends_flash_command_for_single_board(
    monkeypatch: pytest.MonkeyPatch, caplog: pytest.LogCaptureFixture
) -> None:
    monkeypatch.setattr(scan_cli, "ensure_arduino_cli_available", lambda verbose: "arduino-cli Version: 1.0.0")
    monkeypatch.setattr(
        scan_cli,
        "list_detected_ports",
        lambda verbose: [DetectedPort(port="/dev/ttyUSB0", fqbn="arduino:avr:uno", name="Uno")],
    )
    monkeypatch.setattr(scan_cli, "list_installed_cores", lambda: {"arduino:avr"})

    with caplog.at_level(logging.INFO, logger="flexitac"):
        exit_code = scan_cli.main([])
...
def test_scan_reports_no_boards(monkeypatch: pytest.MonkeyPatch, caplog: pytest.LogCaptureFixture) -> None:
    monkeypatch.setattr(scan_cli, "ensure_arduino_cli_available", lambda verbose: "arduino-cli Version: 1.0.0")
    monkeypatch.setattr(scan_cli, "list_detected_ports", lambda verbose: [])
    monkeypatch.setattr(scan_cli, "list_installed_cores", lambda: {"arduino:avr"})
...
def test_scan_reports_unknown_ports(monkeypatch: pytest.MonkeyPatch, caplog: pytest.LogCaptureFixture) -> None:
    ...
    monkeypatch.setattr(scan_cli, "list_installed_cores", lambda: set())
